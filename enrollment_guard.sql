...
CREATE OR REPLACE FUNCTION public.enrollment_guard(p_parent_id uuid, p_program_name text, p_learning_material_name text, p_curriculum_name text, p_grade_level_id uuid DEFAULT NULL::uuid, p_student_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(condition_id uuid, program_id uuid, program_name text, curriculum text, learning_material_name text, requirement_code text, payment_type text, cart_id text, is_completed boolean, completed_at timestamp with time zone, extracted_student_id uuid)
 LANGUAGE plpgsql
AS $function$BEGIN
    RETURN QUERY
    SELECT 
    ...
    LEFT JOIN parent_requirement_progress prp ON 
        ...
        AND (
            ...
            OR prp.student_id = p_student_id 
        )
    WHERE 
